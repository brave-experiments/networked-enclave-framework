...
\begin{figure}[t]
  ...
  bearer token (\ding{202}).  Clients can then request the service provider's
  configuration (\ding{203}).  The application creates an HTTP request, adds the
  confidential bearer token, and sends the request to Cloudflare's API
  (\ding{204}).  Finally, the application asks its hypervisor for an attestation
  document (\ding{205}) and embeds the attestation document in the response to
...
\phw{Elaborate on how users know that the Cloudflare config they see is also the
