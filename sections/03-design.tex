...
the EC2 machine) and (\emph{ii}) the enclave is running the code that the user
...
enclave responds with an attestation document, an only after the client verified
...
once-established TLS session to forego an unnecessary round trip.
...
attest an enclave essentially asks herself ``Does the enclave that's exposed at
...
material.  Instead, enclaves must then synchronize their key material, so they
...
with future enclaves.  If however there are other enclaves, the origin enclave
establishes a connection to a randomly-chosen enclave and initiate key
...
only exchange key material if remote attestation succeeded.  Key synchronization
